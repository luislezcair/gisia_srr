        ...
        public StringBuilder IndextoSolr(string url, string frase, string identificador, bool html, bool pdf, bool rtf, bool word, bool excel, string absolutiUri)
{
    ...
    int index = 1;  //es para darle un id a los archivos, empieza del 1
            ...
            foreach (var file in Directory.GetFiles(url, "*.*"))  //saca los ficheros con todas las extensiones que se encuentren en esa dirección url que pasamos como parametro, la que tiene nombre de usuario/proyecto/.. 
            {
                ...
                using (Stream fileStream = File.OpenRead(file))  //va a ir analizando file x file
                {
                    fileStream.Seek(0, SeekOrigin.Begin); 
                    ExtractResponse extractResponse = solr.Extract(new ExtractParameters(fileStream, fileInfo.Name, url + fileInfo.Name)  //permite extraer el texto con un metodo de solrnet.
                    {
            ...
            if (html) {   //indexo por html
        foreach (var file in Directory.GetFiles(url, "*.htm"))   
      {   
          FileInfo fileInfo = new FileInfo(file);
        ...
        solr.Add(new HtmlContent()  {   //agrego los htmls al solr
            DiscoveryID = index,
            AbsoluteUri = absolutiUri + fileInfo.Name, //coloco el fileinfo.name para que cuando haga click pueda abrir ese archivo
            Text = content,
            Title = fileInfo.Name,
